...
```js
在js中 把正则抽象成了对象  通过new RegExp可以创建正则
 var reg = new RegExp()
  var reg = /字符/; //两个//,如果中间没有是注释,中间随意写个字符就是正则表达式字面量形式
   ...
   特殊字符:
     元字符:
      ...
      限制符:
        {n} 代表前面的字符可以出现n次
        {n,} 代表前面的字符可以出现n次~多次
        {n,m} 代表前面的字符可以出现n次~m次
         * 代表前面的字符出现0~多次
         + 代表前面的字符出现1~多次
         ?  代表前面的字符出现0次或1次
      其他字符:
           [] 中括号占一个字符的位置,在中括号中选一个字符
           [^]中括号占一个字符的位置,不能出现中括号中的字符
           \ 转义符 普通字符转特殊,特殊转普通
           | 或  会把正则分成两部分
           () 分组
           [\u4e00-\u9fa5]  匹配汉字
